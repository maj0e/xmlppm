...
void encoderUsage () {
#ifdef FIXED_MODEL
  fprintf (stderr, "Usage: xmlppm [-m model] [-v] [-s] [-l lev] [infile] [outfile]\n");
#else
...
void decoderUsage () {
#ifdef FIXED_MODEL
  fprintf (stderr, "Usage: xmlunppm [-m model] [-v] [infile] [outfile]\n");
#else
  fprintf (stderr, "Usage: xmlunppm [-m model] [-v] [infile] [outfile]\n");
#endif
  exit (-1);
...
{
#ifdef FIXED_MODEL
  args_t args = { NULL, NULL, NULL, NULL, NULL, NULL, -1, -1, -1};
#else
  ...
  while(argc > 0) {
	...
	{
	  fprintf(stderr,"level must be between 0 and 9");
	  exit(-1);
	}
      argv++;
      argc--;
	...
	fprintf(stderr,"redundant option -m\n");
	encoderUsage();
      }
      argv++;
      argc--;
	...
	encoderUsage();
     ...
     args.modelfile = strdup (argv[0]);
     args.modelfp = fopen_safe (args.modelfile, "r");
     argv++;
     argc--;
     continue;
...
void printArgs(args_t * args) {
  ...
  fprintf(stderr,"Input: %s\nOutput: %s\nModel: %s\nStandalone: %s\nLevel: %d\n",
	  args->infile, args->outfile, args->modelfile, args->standalone? "true" : "false",
	  args->level);
#else
  fprintf(stderr,"Input: %s\nOutput: %s\nStandalone: %s\nLevel: %d\n",
