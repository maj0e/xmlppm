...
struct PPM_MODEL
{
  ...
  void ClearMask ();
  void EncodeChar (FILE * EncodedFile, int c);
  ...
  inline void encodeBinSymbolFixed (PPM_CONTEXT * ctxt, int symbol);	//   BCDE   successor
  inline void encodeSymbol1Fixed (PPM_CONTEXT * ctxt, int symbol);	// other orders:
  inline void encodeSymbol2Fixed (PPM_CONTEXT * ctxt, int symbol);	//   BCD    context
  inline void decodeBinSymbolFixed (PPM_CONTEXT * ctxt);	//    CD    suffix
  inline void decodeSymbol1Fixed (PPM_CONTEXT * ctxt);	//   BCDE   successor
  inline void decodeSymbol2Fixed (PPM_CONTEXT * ctxt);
  inline void update1Fixed (PPM_CONTEXT * ctxt, STATE * p);
  inline void update2Fixed (PPM_CONTEXT * ctxt, STATE * p);
  void EncodeCharFixed (FILE * EncodedFile, int c);
  void PreloadCharFixed (int c);
  int DecodeCharFixed (FILE * EncodedFile);
...
struct PPM_ENCODER : PPM_MODEL {
  ...
  void EncodeCharFixed (int c) {// XXX
    PPM_MODEL::EncodeCharFixed(outfile,c);
  }
};
...
struct PPM_DECODER : public PPM_MODEL {
  ...
  int DecodeCharFixed() {// XXX
    return PPM_MODEL::DecodeCharFixed(infile);
  }
};
