...
inline void
PPM_MODEL::encodeBinSymbolFixed (PPM_CONTEXT * ctxt, int symbol)
{
  BYTE indx = NS2BSIndx[ctxt->Suffix->NumStats] + PrevSuccess + ctxt->Flags;
  STATE & rs = ctxt->ONESTATE;
  WORD & bs =
...
inline void
PPM_MODEL::decodeBinSymbolFixed (PPM_CONTEXT * ctxt)
{
  BYTE indx = NS2BSIndx[ctxt->Suffix->NumStats] + PrevSuccess + ctxt->Flags;
  STATE & rs = ctxt->ONESTATE;
  WORD & bs =
...
inline void
PPM_MODEL::update1Fixed (PPM_CONTEXT * ctxt, STATE * p)
{
  FoundState = p;
  if(!Frozen) {
    p->Freq += 4;
...
inline void
PPM_MODEL::encodeSymbol1Fixed (PPM_CONTEXT * ctxt, int symbol)
{
  UINT LoCnt;
  int i = ctxt->STATS->Symbol;
  STATE *p = ctxt->STATS;
...
inline void
PPM_MODEL::decodeSymbol1Fixed (PPM_CONTEXT * ctxt)
{
  UINT i, count, HiCnt = ctxt->STATS->Freq;
  STATE *p = ctxt->STATS;
  SubRange.scale = ctxt->SUMMFREQ;
...
inline void
PPM_MODEL::update2Fixed (PPM_CONTEXT * ctxt, STATE * p)
{
  FoundState = p;
  if(!Frozen) {
    p->Freq += 4;
...
inline void
PPM_MODEL::encodeSymbol2Fixed (PPM_CONTEXT * ctxt, int symbol)
{
  SEE2_CONTEXT *psee2c = makeEscFreq2 (ctxt);
  int Sym;
  UINT LoCnt = 0, i = ctxt->NumStats - NumMasked;
...
inline void
PPM_MODEL::decodeSymbol2Fixed (PPM_CONTEXT * ctxt)
{
  SEE2_CONTEXT *psee2c = makeEscFreq2 (ctxt);
  UINT Sym, count, HiCnt = 0, i = ctxt->NumStats - NumMasked;
  STATE *ps[256], **pps = ps, *p = ctxt->STATS - 1;
...
void PPM_MODEL::EncodeCharFixed(FILE * EncodedFile, 
				    int c) 
{
  PPM_CONTEXT* MinContext;
  BYTE ns = (MinContext = MaxContext)->NumStats;
  if (ns) {
    encodeSymbol1Fixed (MinContext, c);
    ariEncodeSymbol ();
  }
  else {
    encodeBinSymbolFixed (MinContext, c);
    ariShiftEncodeSymbol (TOT_BITS);
  }
  while (!FoundState) {
    ARI_ENC_NORMALIZE (EncodedFile);
    ...
    encodeSymbol2Fixed (MinContext, c);
    ariEncodeSymbol ();
  }
  if (!OrderFall && (BYTE *) FoundState->Successor >= sa->UnitsStart)
    MaxContext = FoundState->Successor;
...
/* should be identical to EncodeChar except without any encoding actions */
void PPM_MODEL::PreloadCharFixed(int c) 
{
  PPM_CONTEXT* MinContext;
  BYTE ns = (MinContext = MaxContext)->NumStats;
  if (ns) {
...
int PPM_MODEL::DecodeCharFixed(FILE * EncodedFile) 
{
  PPM_CONTEXT *MinContext = MaxContext;
  BYTE ns = MinContext->NumStats;
  int c;
  if (ns) {
    decodeSymbol1Fixed (MinContext);
  } else {
    decodeBinSymbolFixed (MinContext);
  }
  ariRemoveSubrange ();
  while (!FoundState) {
    ARI_DEC_NORMALIZE (EncodedFile);
